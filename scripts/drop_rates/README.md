...
- If the script is given a version name in the CONFIG section at the top, it will save with that name into the site data folder. Otherwise it saves to the `/out` folder.
...
- Item code columns: How many copies of the item are expected per run of the stage.
  - The **Mapping** tab provides the definitions of these item codes.
...
## Misc
- Note: the furthest right column in the Mapping tab is "ALL" (this is an ID, not a word, and it happens to be column 1000 (1-indexed)). If that file is ever committed to the repository, make sure it is appropriately trimmed first.
